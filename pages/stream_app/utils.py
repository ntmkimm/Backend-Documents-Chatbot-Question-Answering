...
from open_notebook.graphs.chat import ThreadState, get_conversation_graph
...
async def fetch_existing_state(chat_session_id: str) -> dict:
    """Fetch the existing state for the given chat session ID"""
    thread_id = chat_session_id or f"thread-{uuid4().hex}"
    config = RunnableConfig(configurable={"thread_id": thread_id})
    graph = await get_conversation_graph(state={})
    existing_state = graph.get_state({"configurable": {"thread_id": chat_session_id}}).values
    return existing_state

async def setup_stream_state(current_notebook: Notebook) -> ChatSession:
    """
    ...
    # Initialize the session state for the current notebook if not already initialized
    ...
    chat_session: Union[ChatSession, None] = (
        await ChatSession.get(current_session_id) if current_session_id else None
    )

    # if there is no chat session, create one or get the first one
    if not chat_session:
        sessions: List[ChatSession] = await current_notebook.get_chat_sessions()
    ...
    # Get the existing state for the session from Langgraph state asynchronously
    existing_state = await fetch_existing_state(chat_session.id)
