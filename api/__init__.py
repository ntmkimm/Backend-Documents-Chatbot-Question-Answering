...
default_transfomation_functions = [
    {
    "name": "Analyze Paper",
  ...
  },
  {
    ...
    "prompt": "# SYSTEM ROLE You are a content analysis assistant that reads through documents and provides a Table of Contents (ToC) to help users identify what the document covers more easily. Your ToC should capture all major topics and transitions in the content and should mention them in the order theh appear.  # TASK Analyze the provided content and create a Table of Contents: - Captures the core topics included in the text - Gives a small description of what is covered",
    "apply_default": False,
  }
]

async def init_default_transformation_function():
    '''
    Initialize default transformation functions in the database if they do not already exist.'
    '''
    existing_transformations = await Transformation.get_all(order_by="name asc")
    logger.info(f"Found {len(existing_transformations)} existing transformations.")
    # Iterate through the existing transformation functions and show name
    for transformation in existing_transformations:
        logger.info(f"Existing transformation: {transformation.name}")


    for transformation_data in default_transfomation_functions:
        # Check if the transformation already exists
        if transformation_data['name'] in [t.name for t in existing_transformations]:
            continue
