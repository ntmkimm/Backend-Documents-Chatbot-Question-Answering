...
from open_notebook.graphs.source_tabular import tabular_source_graph

router = APIRouter()


@router.get("/sources_tabular", response_model=List[SourceListResponse])
async def get_sources_tabular(
...
#     type: str = Field(..., description="Source type: tabular, link, upload, or text")
...
@router.post("/sources_tabular", response_model=SourceResponse)
async def create_source_tabular(source_data: SourceCreate):
    """Create a new source."""
    try:
        ...
        if source_data.type == "tabular":
            if not source_data.file_path:
                raise HTTPException(
                    status_code=400, detail="File path is required for tabular type"
                )
            ...
            if not source_data.content:
                raise HTTPException(
                    status_code=400, detail="Content is required for tabular type"
                )
            content_state["content"] = source_data.content
        else:
            raise HTTPException(
                status_code=400,
                detail="Invalid source type. Must be tabular",
            )
        ...
        # content_state["file_path"], content_state["content"]
        result = await tabular_source_graph.ainvoke(
            {
...
@router.get("/sources_tabular/{source_id}", response_model=SourceResponse)
async def get_source_tabular(source_id: str):
...
@router.put("/sources_tabular/{source_id}", response_model=SourceResponse)
async def update_source_tabular(source_id: str, source_update: SourceUpdate):
    """Update a source."""
    try:
        source = await Source.get(source_id)
        if not source:
...
@router.delete("/sources_tabular/{source_id}")
async def delete_source_tabular(source_id: str):
    """Delete a source."""
    try:
        source = await Source.get(source_id)
        if not source:
...
@router.get("/sources_tabular/{source_id}/insights", response_model=List[SourceInsightResponse])
async def get_source_insights_tabular(source_id: str):
...
@router.post("/sources_tabular/{source_id}/insights", response_model=SourceInsightResponse)
async def create_source_insight_tabular(
    source_id: str,
